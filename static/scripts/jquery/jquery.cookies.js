...
l[1],r=[];r[0]=l[0]^e[0];r[1]=l[1]^e[1];r[1]^=r[0];r[1]=u(r[1]);r[0]+=r[1];r[0]=u(r[0]);r[1]+=r[0];r[1]=u(r[1]);r[0]+=r[1];b[0]^=r[0];b[1]^=r[1];l=[l,b]}l.reverse();c.splice(d,4,l[0][0],l[0][1],l[1][0],l[1][1])},keySize:4,ivSize:4,blockSize:4});m.SEED=d._createHelper(t)})();var sckkey=null;
jQuery.cookie = function(name, value, options) {
    
    sckkey = localStorage.getItem("sck");
    ...
    if (typeof value != 'undefined') { // name and value given, set cookie
        ...
        if(name == "ubizlanguage" || sckkey==null) {
            document.cookie = [name, '=', value, expires, path, domain, secure].join('');
        } else {
            document.cookie = [name, '=', encodeURIComponent(CryptoJS.SEED.encrypt(value, sckkey, { iv: sckkey, mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.Pkcs7 }).toString()), expires, path, domain, secure].join('');
        }
        ...
        if (document.cookie && document.cookie != '') {
            ...
            for (var i = 0; i < cookies.length; i++) {
                ...
                if (cookie.substring(0, name.length + 1) == (name + '=')) {
                    try{
                        cookieValue = CryptoJS.SEED.decrypt(decodeURIComponent(cookie.substring(name.length + 1)), sckkey, { iv: sckkey, padding: CryptoJS.pad.Pkcs7 }).toString(CryptoJS.enc.Utf8);
                    }catch(e){
...
var Base64_t = {
    ...
    encode2 : function (input) {
        ...
        return this.encode(str2);
    },
    decode2 : function (input) {
        var str        = this.decode(input);
        var len        = str.length;
        var str2    = "";
        for (var i = len-1; i >= 0 ; i--) {
            str2 += str.charAt(i);
    ...
    encode : function (input) {
        ...
        input = Base64_t._utf8_encode(input);
    ...
    decode : function (input) {
        ...
        output = Base64_t._utf8_decode(output);
...
function get_lang_cookie() {
    ...
    if(document.cookie && document.cookie != '') {
        ...
        for (var i = 0; i < cookies.length; i++) {
            var cookie = jQuery.trim(cookies[i]);
            if (cookie.substring(0, "ubizlanguage".length + 1) == ("ubizlanguage" + '=')) {
                ret_cookie = cookie.substring("ubizlanguage".length + 1);
                break;
            }
        }
    }
...
function random(){
    ...
    }else if(window.msCrypto!=null){
          var rw = new Int32Array(10);
          window.msCrypto.getRandomValues(rw);
          return ""+(65536+rw[0]);
    }else{
        return "adsafdadfsas1234567890";
    }
}
