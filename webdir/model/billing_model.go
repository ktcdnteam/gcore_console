...
type CDNBlock struct {
	...
	Options struct {
		...
		// Waf                  any `json:"waf"`
		// CountryACL           any `json:"country_acl"`
		...
		// BrowserCacheSettings any `json:"browser_cache_settings"`
		...
		// FollowOriginRedirect any `json:"follow_origin_redirect"`
...
type OptionsBlock struct {
	EdgeCacheSettings  *EdgeCacheSettingsBlock `json:"edge_cache_settings"`
	CacheExpire        any                     `json:"cache_expire"`
...
func GetResourceList() ([]*CDNBlock, error) {
	respBody, err := gcoreapi.Request(nil, "cdn/resources", "GET", loadconf.TOTKENINFO.Access)
	if err != nil {
		log.Println(err)
		return nil, err
	}
	...
	for _, v := range cdnList {
		v.CreatedStr = UTCChangeKR(v.Created)
		log.Printf("%d. %s %s %s \n", v.ID, v.Cname, v.CreatedStr, v.Status)
	}
...
func DeleteCDN(id string) error {
